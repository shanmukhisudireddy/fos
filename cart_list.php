 <!-- Masthead-->
        		...
        		<div class="sticky">
        			<div class="card">
        				<div class="card-body">
							...
							<?php $_SESSION['bill'] = $total;?>

							<br>
							<p><large>Coupon Code</large></p>
							<br>
    ...
    $('#applyCoupon').click(function() {
        ...
        // Send the coupon code and item name to the server for verification
        $.ajax({
            url: 'verify_coupon.php',
            method: 'POST',
            data: { couponCode: couponCode, name: 'burger' }, // Change 'burger' to the specific item name
            dataType: 'json',
            success: function(response) {
                if (response.valid) {
                    ...
                    var discountedAmount = billAmount - (billAmount * discount / 100);

                    $('#discountedAmount').text('Total Amount: ' + discountedAmount.toFixed(2));
                } else {
                    // Coupon code or item is invalid
                    alert('Invalid coupon code or item. Please try again.');
                }
            },
            error: function() {
